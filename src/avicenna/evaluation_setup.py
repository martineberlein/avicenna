...
def evaluate_diagnosis(grammar, evaluation_result: EvaluationResult):
    ...
    y_true = [1 if inp.oracle == OracleResult.BUG else 0 for inp in evaluation_result.evaluation_data]
...
def generate_evaluation_data_set(grammar, oracle, initial_inputs, targeted_size) -> List:
    seed_inputs = set()
    ...
    i = 0
    while not (len(positive_samples) > targeted_size and len(negative_samples) > targeted_size) and i < 3:
        i += 1
